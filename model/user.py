...
def user_profile_of_email(email):
    databse_result = sql_select("SELECT id, name, email, wallet, is_admin, last_login FROM users WHERE email = %s", [email])
    
    return Player(databse_result[0][0], databse_result[0][1], databse_result[0][2], databse_result[0][3], databse_result[0][4])

def user_profile_of_id(id):
    databse_result = sql_select("SELECT id, name, email, wallet, is_admin, last_login FROM users WHERE id = %s", [id])
